...
class StatusUpdate(DataWriterEvent):
    ...
    def handle(self, dispatch):
        # TODO: Probably we should catch sth
        # if someone removes a drive in progress
        ...
        if self.part and self.part_status is not None:
            dispatch.drive_partitions[self.parent][self.part] = \
                self.part_status
            logging.debug(
                _("Partitions status: {0}").format(
                    dispatch.drive_partitions[self.parent]
                )
            )
